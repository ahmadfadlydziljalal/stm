...
namespace app\generators\dzilcrud\generators;
...
class Generator extends \yii\gii\Generator
{
     ...
     * @return string
     */
    public function generateActiveFieldAutoFocus($attribute)
    {
        $tableSchema = $this->getTableSchema();
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
    ...
    public function getDetailColumnNames()
    {
        /* @var $class ActiveRecord */
        $class = $this->modelsClassDetail;
        if (is_subclass_of($class, 'yii\db\ActiveRecord')) {
            return $class::getTableSchema()->getColumnNames();
        } else {
            /* @var $model Model */
    ...
    public function getDetailDetailColumnNames()
    {
        /* @var $class ActiveRecord */
        $class = $this->modelsClassDetailDetail;
        if (is_subclass_of($class, 'yii\db\ActiveRecord')) {
            return $class::getTableSchema()->getColumnNames();
        } else {
            /* @var $model Model */
